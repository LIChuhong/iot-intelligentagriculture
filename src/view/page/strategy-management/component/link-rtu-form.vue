	...
	<div>
		<div v-for="(item,i) in parameterList" :key="i">
			...
			<div v-show="item.parameterType.readWriteType == 1" style="overflow: hidden;padding-bottom: 0.625rem;">
				...
				<span>{{item.parameterName}}:</span><Input v-model="item.parameterType.typeDefaultValue" placeholder="请输入数值" style="width: 60%;float: right;" type="number" />
			</div>
		...
		methods: {
			saveParameterDataList() {
				...
				for(var i = 0;i<list.length;i++){
					if(list[i].check){
						if(list[i].parameterType.typeDefaultValue == ''){
							list[i].parameterType.typeDefaultValue = 0
						}
						parameterDataList.push({
							parameterIndex:i,
							value:Number(list[i].parameterType.typeDefaultValue)
						})
					}
				}
